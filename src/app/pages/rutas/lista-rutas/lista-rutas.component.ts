...
initializeMap(
  ...
  content: `
    <div style="font-family:'Segoe UI',sans-serif;display:inline-block;background:#fff;border-radius:12px;box-shadow:0 4px 12px rgba(0,0,0,.12);padding:6px 8px;line-height:1.2;margin:0;">
      <div style="font-size:14px;color:#2f2f2f;margin:0;">
        <span style="display:block;color:#1aa160;font-weight:600;margin:0;">Punto de Inicio</span>
        <span style="display:block;margin:0;">${labelA || ''}</span>
      </div>
    </div>
  `
});

  markerA.addListener('mouseover', () => infoA.open({ map, anchor: markerA }));
  markerA.addListener('mouseout', () => infoA.close());

  if (hasB && b) {
    const markerB = new google.maps.Marker({ position: b, map, icon: this.markerIconFin });

    const infoB = new google.maps.InfoWindow({
  content: `
    <div style="font-family:'Segoe UI',sans-serif;display:inline-block;background:#fff;border-radius:12px;box-shadow:0 4px 12px rgba(0,0,0,.12);padding:6px 8px;line-height:1.2;margin:0;">
      <div style="font-size:14px;color:#2f2f2f;margin:0;">
        <span style="display:block;color:#d43f3a;font-weight:600;margin:0;">Punto de Destino</span>
        <span style="display:block;margin:0;">${labelB || ''}</span>
      </div>
    </div>
  `
});

    markerB.addListener('mouseover', () => infoB.open({ map, anchor: markerB }));
    markerB.addListener('mouseout', () => infoB.close());
...
getLatFromInicio(row: any): number | null {
  ...
  return Array.isArray(c) && c.length >= 2 ? Number(c[1]) : null; // [lng, lat]
}
