...
export class AltaInstalacionComponent implements OnInit {
  ...
  private pendingSelecciones: { idDispositivo?: number; idBlueVox?: number; idVehiculo?: number } = {};
  ...
  private limpiarDependientes(): void {
    const opts = { emitEvent: false };
    this.instalacionesForm.patchValue({ idDispositivo: null, idBlueVox: null, idVehiculo: null }, opts);
  ...
  private suscribirCambioEquipos(): void {
    this.instalacionesForm.get('idDispositivo')?.valueChanges.subscribe(async (nuevo: any) => {
    ...
    this.instalacionesForm.get('idBlueVox')?.valueChanges.subscribe(async (nuevo: any) => {
      if (this.bootstrapping || !this.idInstalacion) return; // solo en edición
    ...
    `,
      ...
      didOpen: () => {
        ...
        const styleInput = (el: HTMLElement | null, isSelect = false) => {
          ...
          el.style.transition = 'border-color .15s ease, box-shadow .15s ease, background .15s ease';
          el.addEventListener('focus', () => { el.style.borderColor = '#7aa2ff'; el.style.boxShadow = '0 0 0 3px rgba(122,162,255,.25)'; el.style.background = '#0e1521'; });
          el.addEventListener('blur', () => { el.style.borderColor = '#213041'; el.style.boxShadow = 'none'; el.style.background = '#0b121b'; });
        };

        const selectEl = document.getElementById('estado-select') as HTMLSelectElement | null;
        const inputEl = document.getElementById('comentarios-input') as HTMLInputElement | null;
        styleInput(selectEl, true);
        styleInput(inputEl, false);

        // Ajuste SOLO del input: más angosto + centrado
        if (inputEl) {
      ...
      preConfirm: () => {
        ...
        if (!estadoStr) { Swal.showValidationMessage('Selecciona un estado'); return false as any; }
      ...
      .subscribe({
        next: (resp: any) => {
          ...
          this.listaDipositivos = this.ensureSelectedOptionVisible(
            this.listaDipositivos, this.pendingSelecciones?.idDispositivo, this.pendingLabels.dispositivo, 'numeroSerie'
          );

          if (!this.listaBlueVox?.length) this.listaBlueVox = [];
          this.listaBlueVox = this.ensureSelectedOptionVisible(
            this.listaBlueVox, this.pendingSelecciones?.idBlueVox, this.pendingLabels.bluevox, 'numeroSerieBlueVox'
          );
          ...
          if (applyPending) {
            ...
            f.get('idDispositivo')?.setValue(n(this.pendingSelecciones.idDispositivo), { emitEvent: false });
            f.get('idBlueVox')?.setValue(n(this.pendingSelecciones.idBlueVox), { emitEvent: false });
        ...
        error: (err) => {
          ...
          this.listaDipositivos = this.ensureSelectedOptionVisible(
            [], this.pendingSelecciones?.idDispositivo, this.pendingLabels.dispositivo, 'numeroSerie'
          );
          this.listaBlueVox = this.ensureSelectedOptionVisible(
            [], this.pendingSelecciones?.idBlueVox, this.pendingLabels.bluevox, 'numeroSerieBlueVox'
          );
          this.listaVehiculos = this.ensureSelectedOptionVisible(
          ...
          const f = this.instalacionesForm;
          const n = (v: any) => (v == null ? null : Number(v));
          if (this.pendingSelecciones.idDispositivo != null) f.get('idDispositivo')?.setValue(n(this.pendingSelecciones.idDispositivo), { emitEvent: false });
          if (this.pendingSelecciones.idBlueVox != null) f.get('idBlueVox')?.setValue(n(this.pendingSelecciones.idBlueVox), { emitEvent: false });
  ...
  obtenerInstalacion(): void {
    ...
    this.instService.obtenerInstalacion(this.idInstalacion).subscribe((response: any) => {
      ...
      if (idCliente) {
        ...
        this.listaBlueVox = this.ensureSelectedOptionVisible([], idBlueVox, this.pendingLabels.bluevox, 'numeroSerieBlueVox');
  ...
  agregar(): void {
    ...
    if (this.instalacionesForm.invalid) {
      ...
      const etiquetas: any = { idDispositivo: 'Dispositivo', idBlueVox: 'Bluevox', idVehiculo: 'Vehículo', idCliente: 'Cliente' };
      const camposFaltantes: string[] = [];
      Object.keys(this.instalacionesForm.controls).forEach((key) => {
        const control = this.instalacionesForm.get(key);
        if (control?.invalid && control.errors?.['required']) camposFaltantes.push(etiquetas[key] || key);
      });
