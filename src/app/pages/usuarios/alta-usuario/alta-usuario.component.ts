...
export class AltaUsuarioComponent implements OnInit {
  ...
  // Helper para obtener el ID de permiso sin importar el nombre de la propiedad
  ...
  onToggle(permiso: any, checked: boolean) {
    ...
    if (idNum === null) return; // nada que hacer si no hay ID válido
  ...
  obtenerUsuarioID() {
    this.usuaService.obtenerUsuario(this.idUsuario).subscribe((response: any) => {
      ...
      // Acceso correcto a la estructura
  ...
  isPermisoAsignado(id: any): boolean {
    const nid = this.getPermisoId({ idPermiso: id, id }); // acepta idPermiso o id
    return nid !== null && Array.isArray(this.permisosIds) && this.permisosIds.includes(nid);
  }
  ...
  // Reemplaza tu uploadLogoAuto() por esto (o llámalo 'upload' si quieres)
  private uploadLogoAuto(): void {
    // arma el FormData igual que en Postman
    const fd = new FormData();

    if (this.logoFile) {
      // caso: el usuario seleccionó un archivo local
      fd.append('file', this.logoFile, this.logoFile.name);
    } else {
      // caso: NO seleccionaron archivo -> manda la URL por la misma clave 'file'
      fd.append('file', this.DEFAULT_AVATAR_URL);
    }

    // ajusta si tu folder/idModule cambian
  ...
  next: (res: any) => {
    ...
    // toma lo que venga como URL (tu backend manda "url" según la captura)
    const url = res?.url ?? res?.Location ?? res?.data?.url ?? '';

    if (url) {
      // **se va directo al formulario**
      this.usuarioForm.patchValue({ fotoPerfil: url });

      // opcional: si es imagen, actualiza la previsualización
