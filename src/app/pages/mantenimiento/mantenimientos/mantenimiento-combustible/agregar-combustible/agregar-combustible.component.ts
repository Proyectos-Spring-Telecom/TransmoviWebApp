...
export class AgregarCombustibleComponent implements OnInit {
    ...
    this.combuService
      ...
      .subscribe((response: any) => {
        ...
        const cantidadDisplay =
          data.cantidadCombustible != null
            ? `${data.cantidadCombustible} L`
            : null;

        const kilometrajeDisplay =
          data.kilometraje != null ? `${data.kilometraje} Km` : null;

        const precioDisplay =
          data.precioCombustible != null
            ? `$${this.parseCosto(data.precioCombustible).toFixed(2)}`
            : null;

        this.manCombustibleForm.patchValue({
          idTipoCombustible:
            idTipoCombustibleRaw != null ? Number(idTipoCombustibleRaw) : null,
          cantidadCombustible: cantidadDisplay,
          precioCombustible: precioDisplay,
          idInstalacion:
            idInstalacionRaw != null ? Number(idInstalacionRaw) : null,
          estatus: data.estatus ?? 1,
          fechaHora: data.fechaHora,
          kilometraje: kilometrajeDisplay,
  ...
  agregar() {
    ...
    const payload = {
      ...
      cantidadCombustible: this.parseDecimal(formValue.cantidadCombustible),
      ...
      kilometraje: this.parseDecimal(formValue.kilometraje),
  ...
  onCantidadFocus(): void {
    const c = this.manCombustibleForm.get('cantidadCombustible');
    if (!c) return;
  ...
  onKilometrajeFocus(): void {
    const c = this.manCombustibleForm.get('kilometraje');
    if (!c) return;
    const raw = (c.value ?? '').toString();
    c.setValue(raw.replace(/[^0-9.,-]/g, '').replace(',', '.'));
  }

  onKilometrajeBlur(): void {
    const c = this.manCombustibleForm.get('kilometraje');
    if (!c) return;
    const raw = (c.value ?? '').toString().replace(/[^0-9.,-]/g, '').replace(',', '.');
    const num = parseFloat(raw);
  ...
  onCostoFocus(): void {
    const c = this.manCombustibleForm.get('precioCombustible');
    if (!c) return;
    const raw = (c.value ?? '').toString();
    c.setValue(raw.replace(/[^0-9.,-]/g, '').replace(',', '.'));
  }
